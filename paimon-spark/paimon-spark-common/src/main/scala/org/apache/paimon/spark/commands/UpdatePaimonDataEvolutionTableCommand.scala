...
case class UpdatePaimonDataEvolutionTableCommand(
  ...
  private def deterministicUpdate: Boolean = {
    condition.deterministic && alignedExpressions.forall { case (expression, _) =>
    ...
    if (retryWaitMs > 0) {
      try {
        ...
        case e: InterruptedException =>
          ...
          throw new RuntimeException("Interrupted while retrying data-evolution UPDATE conflict.", e)
      }
    }
  }
